...
def toGameForm (g : AugmentedForm) (h : TombstoneFree g) : GameForm := by
  unfold TombstoneFree at h
  obtain ⟨_, _, h_moves⟩ := h
  exact !{Set.range (fun gl : gᴸ => toGameForm gl (h_moves .left gl gl.property)) |
          Set.range (fun gr : gᴿ => toGameForm gr (h_moves .right gr gr.property))}
termination_by g
decreasing_by form_wf
...
theorem ofGameForm_tombstoneFree (g : GameForm) : TombstoneFree (ofGameForm g) := by
  ...
  | mk g ih =>
    unfold TombstoneFree ofGameForm
    constructor
    · simp only [tombstone_ofSetsWithTombs]; simp
    constructor
    · simp only [tombstone_ofSetsWithTombs]; simp
    · intro p gp h_mem
      simp [moves_ofSetsWithTombs] at h_mem
      obtain ⟨gp_orig, h_gp_orig, rfl⟩ := h_mem
      exact ih p gp_orig h_gp_orig
...
private theorem add_zero' (x : AugmentedForm) : x + 0 = x := by
  ...
  ext
  · aesop
  · have := tombstone_add_zero x Player.left
    exact this
  · have := tombstone_add_zero x Player.right
    exact this
