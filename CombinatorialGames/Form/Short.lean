...
protected theorem of_mem_moves {x y : G} (hx : IsShort x) {p} (hy : y ∈ Moves.moves p x) : IsShort y :=
  (isShort_def.mp hx p).right y hy

protected theorem isOption {x y : G} (hx : IsShort x) (h : Moves.IsOption y x) : IsShort y := by
  rw [Moves.IsOption.iff_mem_union] at h
  cases h with
  | inl h => exact IsShort.of_mem_moves hx h
  | inr h => exact IsShort.of_mem_moves hx h

alias _root_.Moves.IsOption.isShort := IsShort.isOption

protected theorem subposition {x y : G} (hx : IsShort x) (h : Subposition y x) : IsShort y := by
  cases h with
  | single h => exact IsShort.isOption hx h
  | tail IH h => have hx' := IsShort.isOption hx h; exact IsShort.subposition hx' IH
termination_by x
decreasing_by form_wf

alias _root_.Moves.IsOption.subposition := IsShort.subposition
...
theorem finite_setOf_subposition {x : G} (hx : IsShort x) : {y | Moves.Subposition y x}.Finite := by
  ...
  exact finite_setOf_subposition (IsShort.isOption hx hy)
termination_by x
decreasing_by form_wf
...
protected theorem add {x y : G} (hx : IsShort x) (hy : IsShort y) : IsShort (x + y) := by
  refine IsShort.mk fun p ↦ ⟨?_, ?_⟩
  · change (moves p (x + y)).Finite; rw [moves_add]
    simpa using ⟨(IsShort.finite_moves _ hx).image _, (IsShort.finite_moves _ hy).image _⟩
  · intro z hz; change z ∈ moves p (x + y) at hz; rw [moves_add] at hz
    cases hz with
    | inl h => obtain ⟨a, ha, rfl⟩ := h; exact IsShort.add (IsShort.of_mem_moves hx ha) hy
    | inr h => obtain ⟨b, hb, rfl⟩ := h; exact IsShort.add hx (IsShort.of_mem_moves hy hb)
termination_by (x, y)
decreasing_by form_wf

protected theorem neg {x : G} (hx : IsShort x) : IsShort (-x) := by
  rw [isShort_def]; intro p; constructor
  · change (moves p (-x)).Finite; rw [moves_neg]
    simpa [← Set.image_neg_eq_neg] using (IsShort.finite_moves _ hx).image _
  · intro y hy; change y ∈ moves p (-x) at hy; rw [moves_neg] at hy
    have h_neg_y : -y ∈ moves (-p) x := by rwa [Set.mem_neg] at hy
    simpa using IsShort.neg (IsShort.of_mem_moves hx h_neg_y)
termination_by x
decreasing_by form_wf
...
instance : Hereditary IsShort (G := G) where
  has_option := Moves.IsOption.isShort
...
protected theorem intCast : ∀ n : ℤ, IsShort (n : G)
  | .ofNat n => IsShort.natCast n
  | .negSucc n => IsShort.neg (IsShort.natCast (n + 1))

end IsShort

-- TODO: Move to Short.lean
class Short (A : G → Prop) where
...
theorem short_iff_birthday_finite {g : GameForm} :
  ...
  · have (y : {y // IsOption y g}) : ∃ n : ℕ, birthday y.val = n := by
      ...
      exact IsShort.isOption h y.prop
    choose f hf using this
    have : Finite { y // IsOption y g } := (IsShort.finite_setOf_isOption h).to_subtype
    ...
    aesop
  · rw [NatOrdinal.lt_omega0, IsShort.short_iff_finite_setOf_subposition]
    intro ⟨n, hn⟩
