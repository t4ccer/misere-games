...
private def WinsGoingFirst' (p : Player) (g : AugmentedForm) : Prop :=
  IsEndLike p g ∨ (∃ g', ∃ (_ : g' ∈ Form.moves p g), ¬WinsGoingFirst' (-p) g')
termination_by g
...
private theorem EndLike_WinsGoingFirst' {g : AugmentedForm} {p : Player} (h1 : IsEndLike p g)
    : WinsGoingFirst' p g := by
  rw [WinsGoingFirst']
  exact Or.inl h1
...
private theorem WinsGoingFirst_iff' {g : AugmentedForm} {p : Player}
    : WinsGoingFirst' p g ↔ (IsEndLike p g) ∨ (∃ g' ∈ Form.moves p g, ¬WinsGoingFirst' (-p) g') := by
  rw [WinsGoingFirst']
  simp only [exists_prop]
...
@[no_expose] noncomputable instance : MisereForm AugmentedForm where
  ...
  WinsGoingFirst_iff _p _g := WinsGoingFirst_iff'
