...
theorem treeSize_lt_of_mem_moves {g x : AugmentedForm.{u}} {p : Player}
    (hg : IsShort g) (hx : x ∈ moves p g) : treeSize x < treeSize g := by
  cases p
  · have h_treeSize_x :
        treeSize x ≤ ∑ y ∈ (Short.finite_moves Player.left hg).toFinset, treeSize y :=
      Finset.single_le_sum (fun y _ => NatOrdinal.zero_le (treeSize y)) (by simpa using hx)
    exact lt_of_le_of_lt h_treeSize_x (by simpa using sum_leftMoves_add_one_le hg)
  · rw [← AugmentedForm.treeSize_neg]
    have h_neg_x : -x ∈ moves .left (-g) := by
      rw [moves_neg, Set.mem_neg, neg_neg]; exact hx
    have h_neg_x_treeSize : (-x).treeSize
        ≤ (Short.finite_moves .left (Short.neg hg)).toFinset.sum (fun y => y.treeSize) :=
      Finset.single_le_sum (fun y _ => NatOrdinal.zero_le _) (by simpa using h_neg_x)
    exact lt_of_le_of_lt h_neg_x_treeSize
      (by simpa [AugmentedForm.treeSize_neg] using sum_leftMoves_add_one_le (Short.neg hg))
